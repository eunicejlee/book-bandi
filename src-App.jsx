...
function CameraScreen({ scan, onTap, collected, bookFont, onBack, sheetOpen }) {
    ...
    <div className="cb-cam">
      ...
      {!sheetOpen && scan === "done" && (
        <div className="cb-cam-hint">
          ...
          <div>
            <b>궁금한 단어를 눌러봐.</b>
            <span>점선이 그어진 말은 바로 눌러볼 수 있어!</span>
          </div>
        </div>
      )}
    </div>
...
.cb-viewfinder{position:relative; flex:1; margin:0 12px 12px; overflow:hidden; border-radius:22px;}
...
.cb-cam-hint{
  position:absolute; left:16px; right:16px; bottom:104px; z-index:6;
  background:rgba(255,255,255,.96); border-radius:20px; padding:12px 14px;
  display:flex; align-items:center; gap:10px; box-shadow:0 12px 30px rgba(0,0,0,.3);
  animation:cb-up .35s ease both;
}
.cb-cam-hint b{font-family:'Jua',sans-serif; font-size:15px; font-weight:400; display:block;}
.cb-cam-hint span{font-size:12px; color:var(--ink-2);}
